...
from ml_engine.hybrid_ranker import compute_hybrid_score, compute_confidence
...
def get_personalized_recommendations(
    ...
    personalized = recommendation_service.get_personalized(liked_ids, genres, top_n=top_n + len(downvoted))
    personalized = [m for m in personalized if m["id"] not in downvoted][:top_n]
    if not personalized:
        trending = recommendation_service.get_trending_fallback(top_n=top_n)
        trending = [m for m in trending if m["id"] not in downvoted]
...
async def get_hybrid_recommendations(
    ...
    if session_id:
        downvoted = user_service.get_downvoted_movie_ids(db, session_id)
        candidates = [c for c in candidates if c["id"] not in downvoted]
    ...
    for c in candidates:
        ...
        reviews = await fetch_reviews(c["id"])
        sentiment_pct = None
