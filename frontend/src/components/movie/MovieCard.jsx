...
export default function MovieCard({ movie, matchPercentage, confidence, onFeedback }) {
  ...
  const detailPath = movie.source === "tmdb" || movie.source === "tmdb_external" ? `/movie/tmdb/${movie.id}` : `/movie/${movie.id}`;

  return (
    <Link to={detailPath} className="shrink-0 w-48 md:w-56 group">
      <motion.div
        whileHover={{ y: -6 }}
      ...
      >
        ...
        <div className="p-3">
          ...
          {movie.reasons?.[0] && (
            <p className="text-smoke text-[10px] mt-1 line-clamp-1" title={movie.reasons.join(" • ")}>
              {movie.reasons[0]}
            </p>
          )}
          {feedbackGiven === "down" && (
